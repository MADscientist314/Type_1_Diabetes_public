...
metadata <- read.table('metadataT1D.txt',sep='\t',h=T,row.names=1,check=F,comment='')
metadata
# load OTU table
...
otus <- read.table('otusT1D.txt',sep='\t', header=T,row.names=1,check=F,skip=1,comment='')
...
BiocManager::install("Sourcetracker")
