...
#sam<-read.csv(file = "metadata_table.csv", header = T, sep = ",",row.names = 1)
#sample_data(ASV_physeq_core)<-sample_data(sam)
analyze_st<-function(filename,st,var1,var2) 
{
  #filename<-tax_glom(physeq = filename, taxrank = "Genus", NArm = F)
  s<-as.character(meta(filename)$st)
  count_tab<-as.data.frame(otu_table(filename))
  tax_tab<-as.matrix(tax_table(filename))
  sample_info_tab<-as.data.frame(sample_data(filename))
  ...
  plot(euc_dend, ylab="VST Euc. dist.", main = as.character(s))
  ...
  # 
  # plot_ordination(vst_physeq, vst_pcoa, color=st) + 
  #   geom_point(size=3) + labs(col=vary) + 
  ...
  deseq <- DESeq(deseq)
  contrast<-as.character(c(st,var2,var1))
  ...
  deseq_res <- results(deseq, alpha=0.01, contrast=contrast)
  ...
  #                 lab = paste(sigtab_deseq_with_tax$Genus,"_",rownames(sigtab_deseq_with_tax),sep = ""),
  #                 x = 'log2FoldChange',
  #                 y = 'pvalue',
  #                 xlim = c(-8, 8),
  ...
  # # ########Modified cutoffs for pvalue and log2FC
   EnhancedVolcano(res1,
                   lab = sigtab_deseq_with_tax$Genus,#paste(sigtab_deseq_with_tax$Genus,"_",rownames(sigtab_deseq_with_tax),sep = ""),
                   x = 'log2FoldChange',
                   y = 'pvalue',
                   xlim = c(-8, 8),
                   title =  paste(st,' Control versus Type 1 Diabetes'),
...
ASV_physeq_core_deliv<-subset_samples(ASV_physeq_core, Delivery!="NA")

########HERE IS HOW TO DO THE FUNCTION#########
#analyse_st(physeq_object,contrast,variable1, variable2)
...
analyze_st(ASV_physeq_core_deliv,"Delivery","Vaginal","Csection")
#analyse_st(physeq_object,'factorName','numeratorLevel','denominatorLevel')
