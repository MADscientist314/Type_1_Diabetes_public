...
getwd()
#sam<-read.csv(file = "metadata_table.csv",sep = ",", header = T, row.names = 1)
#sample_data(ASV_physeq_core)<-sample_data(sam)
#write_phyloseq(ASV_physeq_core, type = "METADATA")
# #and lets make a new phyloseq object for each sampletype
analyze_st<-function(filename){
...
capture.output(anova(betadisper(euc_dist, sample_info_tab$disease)),file = "euc_vst_disease_within_group_anova_betadisper.tsv")
#Perumtational test for significance
capture.output(adonis(euc_dist~sample_info_tab$disease),file = "euc_vst_disease_adonis.tsv")# 0.003
a<-adonis(euc_dist~sample_info_tab$disease, permutations = 9999)
library(mosaic)
msummary(a$aov.tab)
