...
derep_forward<-readRDS("derep_forward.RDS")
dada_forward<-readRDS("dada_forward.RDS")
derep_reverse<-readRDS("derep_reverse.RDS")
dada_reverse<-readRDS("dada_reverse.RDS")
merged_amplicons <- mergePairs(dada_forward, derep_forward, dada_reverse,derep_reverse, trimOverhang=TRUE, minOverlap=59,verbose = T)# this object holds a lot of information that may be the first place you'd want to look if you want to start poking under the hood
